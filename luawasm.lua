	...
	for i, import in ipairs(self.imports or {}) do
		...
		if import.importType == EXTERN_FUNC then
			if type(importDef) ~= "function" then
				error(
					string.format(
						"Import for `%s.%s` is of incorrect type (got %s, expected function)",
						import.modName,
						import.name,
						type(importDef)
					)
			...
			elseif exportDef.funcKind == "inst" then
				inst.exports[export.name] = function(...)
					return inst:evaluate(
						exportDef.code.expr,
						#exportDef.funcType.returns,
						#exportDef.funcType.args,
						...
					)
				end
			end
...
function instanceIndex:evaluate(instrSeq, returns, args, ...)
	...
	local function doBlock(instrSeq, argc, returnc, labelPos)
		...
		labels[#labels + 1] =
			{ instrSeq = seq, instrPos = labelPos or pos, endPos = pos, stack = stack, returns = returnc }
		stack = {}
		pushn(args)
		seq = instrSeq
		pos = 1
	...
	local function readInstrSeq()
		...
		while true do
			...
			elseif
				...
				or opcode == 0x1A
				or opcode == 0x1B
				or opcode >= 0x45 and opcode <= 0xC4
			then
			...
			elseif
				...
				or opcode == 0x10
				or opcode == 0xD2
				or opcode >= 0x20 and opcode <= 0x26
			then
